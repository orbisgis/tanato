...
import com.vividsolutions.jts.operation.buffer.OffsetCurveVertexList;
 ...
 * @version 1.7
 */
public class OffsetCurveBuilder3D 
{  
  /**
      ...
      if (offset0.p1.distance(offset1.p0) < distance
        ...
        if (closingSegFactor > 0) {
          Coordinate mid0 = new Coordinate((closingSegFactor * offset0.p1.x + s1.x)/(closingSegFactor + 1), 
              (closingSegFactor*offset0.p1.y + s1.y)/(closingSegFactor + 1),offset0.p1.z + s1.z);
          vertexList.addPt(mid0);
          Coordinate mid1 = new Coordinate((closingSegFactor*offset1.p0.x + s1.x)/(closingSegFactor + 1), 
             (closingSegFactor*offset1.p0.y + s1.y)/(closingSegFactor + 1),offset1.p1.z + s1.z);
  ...
  private void addLineEndCap(Coordinate p0, Coordinate p1)
  {
    ...
    switch (bufParams.getEndCapStyle()) {
      case BufferParameters.CAP_ROUND:
        ...
        addFillet(p1, angle + Math.PI / 2, angle - Math.PI / 2, CGAlgorithms.CLOCKWISE, distance,offsetL.p1.z,offsetR.p1.z);
      ...
      case BufferParameters.CAP_SQUARE:
        ...
        if (dz==0)
        {
        	squareCapLOffset = new Coordinate(
        ...
        else
        {
            squareCapLOffset = new Coordinate(
            		offsetL.p1.x + squareCapSideOffset.x,
            		offsetL.p1.y + squareCapSideOffset.y,
            		offsetL.p1.z +distance*dz/dist);
        }

        Coordinate squareCapROffset;
        if (dz==0)
        {
        	squareCapROffset= new Coordinate(
...
//  		addBevelJoin(offset0, offset1);
  		...
  		double mitreLimit)
  {
  	...
  	double bevelMidZ = seg0.p0.z + seg1.p1.z;
  ...
  private void addFillet(Coordinate p, Coordinate p0, Coordinate p1, int direction, double radius)
  {
    ...
    vertexList.addPt(p0);
    addFillet(p, startAngle, endAngle, direction, radius, p0.z ,p1.z);
    vertexList.addPt(p1);
  }
   ...
   * @param radius the radius of the fillet
   */
  private void addFillet(Coordinate p, double startAngle, double endAngle, int direction, double radius, double p0z,double p1z)
  {
  ...
  {
    // add start point
    Coordinate pt = new Coordinate(p.x + distance, p.y,p.z);
    vertexList.addPt(pt);
    addFillet(p, 0.0, 2.0 * Math.PI, -1, distance,p.z,p.z);
  ...
  {
    // add start point
  	vertexList.addPt(new Coordinate(p.x + distance, p.y + distance,p.z));
  	vertexList.addPt(new Coordinate(p.x + distance, p.y - distance,p.z));
  	vertexList.addPt(new Coordinate(p.x - distance, p.y - distance,p.z));
  	vertexList.addPt(new Coordinate(p.x - distance, p.y + distance,p.z));
  	vertexList.addPt(new Coordinate(p.x + distance, p.y + distance,p.z));
